...
| 20260619-v17 | 2026-06-19 | **Carril rápido para modificaciones T1** (§8.3.2): refina los tiers v14 reduciendo la *ceremonia* (no solo el rigor) de los cambios de pura presentación. Para T1, discovery y CHANGE-SET pasan a **opcionales**, la edición de spec se reduce a **una línea de changelog** (+ nota de sección solo si llena un hueco real de doc), y las confirmaciones se **colapsan a una sola aprobación inicial** (tier + enfoque + delta en 1 línea), seguida de código → checks inline → gate de prueba manual → commit. Nuevo **test de entrada objetivo** con exclusión explícita de componentes compartidos/foundacionales (si el fix sería tocar algo compartido → §8.4 o sube de tier; un override local sí califica). Innegociables intactos: gate de prueba manual y válvula de escape. Tabla "Qué pasos ejecuta cada tier" actualizada (celdas T1 de Discovery y `/sdd-modify-spec`). Alineado `protocols/tier-routing.md`. **Pendiente de alineación (decisión del autor: diferido):** prompts `07-modify-spec` y `04-codegen` para que el flujo colapse las confirmaciones en las corridas. Motivación: los tiers v14 bajaron el rigor pero la ceremonia del modify-spec seguía corriendo completa en T1; un cambio cosmético real (encabezados de tabla, ~4 clases CSS) recorrió discovery + CHANGE-SET + verify report para un override local. |
...
| 20260815-v18 | 2026-08-15 | **Modelo por defecto de las pasadas adversarias → Sonnet** (§11.1). Los subagentes de `/sdd-adversarial-spec` y `/sdd-adversarial-code` pasan a lanzarse con `model: sonnet` fijado en la llamada a la tool `Agent` de cada comando (antes heredaban el modelo de la sesión, típicamente Opus). La pasada adversaria es razonamiento estructurado donde Sonnet rinde a una fracción del costo/latencia; la de código además corre ×4 capas (costo dominante del ciclo). No afecta el modelo de la sesión principal; el default es reversible a `opus` por comando si al medir se pierden hallazgos reales (Regla 5). Efecto secundario buscado: **independencia de modelo** del adversario respecto del generador, sobre la independencia de contexto que ya daba el subagente limpio. Alineados los comandos `/sdd-adversarial-spec` y `/sdd-adversarial-code`; `plugin.json` a 1.1.3. Micro-opt adjunta en `/sdd-adversarial-spec` Paso 1: leer solo la §10.2 + header de versión de la spec en la sesión orquestadora, no el documento completo (la spec entera es trabajo del subagente). Motivación: las pasadas adversarias seguían siendo el costo/latencia dominante; el modelo es la palanca nativa que no resigna la revisión estructurada ni saca datos de Anthropic. |
